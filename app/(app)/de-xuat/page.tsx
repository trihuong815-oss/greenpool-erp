// Phase 11 — Module Đề xuất (proposals). Tách khỏi /giao-viec.
// Proposal = quy trình xin duyệt (draft → submitted → approved/rejected).
// Approve → auto-tạo task ở /giao-viec với link 2 chiều.
...
export default async function DeXuatPage() {
  ...
  // Lookup tables cho dropdown form
  const db = getFirebaseAdminDb();
  const [deptSnap, branchSnap, userSnap, roleSnap] = await Promise.all([
  ...
  const departments = deptSnap.docs.map((d) => {
    const x = d.data();
    return { id: d.id, name: x.name ?? d.id, blockId: (x.block_id ?? null) as 'KD' | 'VP' | null };
  });
  ...
  const roles = roleSnap.docs.map((d) => ({ code: d.id, name: d.data().name ?? d.id }));

  return (
    <>
      <AppTopBar
        title="Đề xuất"
        subtitle="Hồ sơ xin duyệt — duyệt xong tự sinh nhiệm vụ thực hiện"
        icon="task"
      />
