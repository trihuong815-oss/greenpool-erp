...
const STATUS_LABEL: Record<TaskStatus, string> = {
  pending_approval: 'ChÃ¡Â»Â duyÃ¡Â»Ât',
  pending: 'ChÃ¡Â»Â lÃÂ m',
  in_progress: 'ÃÂang lÃÂ m',
  requested_revision: 'YÃÂªu cÃ¡ÂºÂ§u bÃ¡Â»Â sung',
  done: 'HoÃÂ n thÃÂ nh',
...
const PRIORITY_LABEL: Record<string, string> = {
  low: 'ThÃ¡ÂºÂ¥p', normal: 'BÃÂ¬nh thÃÂ°Ã¡Â»Âng', high: 'Cao', urgent: 'KhÃ¡ÂºÂ©n',
};
...
const BLOCK_LABEL: Record<string, { label: string; bg: string }> = {
  ...
  all: { label: 'ToÃÂ n cÃÂ´ng ty', bg: 'bg-slate-100 text-slate-700' },
};
...
export function GiaoViecClient(props: Props) {
  ...
  // Per-dept stats for "CÃÂ´ng viÃ¡Â»Âc theo khÃ¡Â»Âi"
  const perDeptStats = useMemo(() => {
    ...
    allTasks.forEach((t) => {
      ...
      const name = t.assigneeDeptId
        ...
        : (t.assigneeFacilityId ? (branches.find((b) => b.id === t.assigneeFacilityId)?.name ?? t.assigneeFacilityId) : 'CÃÂ¡ nhÃÂ¢n');
  ...
  const tabDef: { key: TabKey; label: string; badge?: number }[] = [
    { key: 'my-tasks', label: 'TÃÂ´i phÃ¡Â»Â¥ trÃÂ¡ch' },
    { key: 'assigned-by-me', label: 'TÃÂ´i giao' },
    ...(showLienKhoiTab ? [{ key: 'cross-block' as TabKey, label: 'LiÃÂªn khÃ¡Â»Âi' }] : []),
    { key: 'pending-response', label: 'ChÃ¡Â»Â phÃ¡ÂºÂ£n hÃ¡Â»Âi', badge: approvalCount || undefined },
    { key: 'overdue', label: 'QuÃÂ¡ hÃ¡ÂºÂ¡n', badge: kpi.overdue || undefined },
  ];
  return (
    <div className="max-w-7xl mx-auto space-y-5">

      {/* ===== HEADER: TÃ¡Â»Âng quan hÃÂ´m nay ===== */}
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">TÃ¡Â»Âng quan hÃÂ´m nay</h2>
          ...
          <div className="flex items-center gap-2">
            {(canCreateAssignment || canCreateProposal) && (
              ...
              >
                <Plus size={15} /> TÃ¡ÂºÂ¡o ÃÂiÃ¡Â»Âu phÃ¡Â»Âi
              </button>
            )}
            <button onClick={refresh} className="p-2 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition" title="LÃÂ m mÃ¡Â»Âi">
              <RefreshCw size={15} />
            </button>
          </div>
        </div>

        {/* KPI cards Ã¢ÂÂ 5 ÃÂ´ theo mockup */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <KpiCard label="ÃÂang xÃ¡Â»Â­ lÃÂ½" value={kpi.inProgress} icon={Clock} accent="sky" sub={kpi.inProgress > 0 ? `+${Math.round(kpi.inProgress/Math.max(allTasks.length,1)*100)}% tÃ¡Â»Âng` : undefined} />
          <KpiCard label="ChÃ¡Â»Â phÃ¡ÂºÂ£n hÃ¡Â»Âi" value={kpi.pendingApproval} icon={ShieldCheck} accent={kpi.pendingApproval > 0 ? 'amber' : 'slate'} />
          <KpiCard label="ChÃ¡Â»Â duyÃ¡Â»Ât" value={kpi.pendingDone} icon={AlertTriangle} accent={kpi.pendingDone > 0 ? 'orange' : 'slate'} />
          <KpiCard label="QuÃÂ¡ hÃ¡ÂºÂ¡n" value={kpi.overdue} icon={AlertTriangle} accent={kpi.overdue > 0 ? 'rose' : 'slate'} />
          <KpiCard label="HoÃÂ n thÃÂ nh" value={kpi.done} icon={CheckCircle2} accent="emerald" sub={allTasks.length > 0 ? `+${Math.round(kpi.done/allTasks.length*100)}% tÃ¡Â»Âng` : undefined} />
        </div>
      </section>

      {/* ===== HÃÂNG 2: CÃÂ´ng viÃ¡Â»Âc theo khÃ¡Â»Âi + TÃ¡ÂºÂ¯c nghÃ¡ÂºÂ½n + QuÃÂ¡ hÃ¡ÂºÂ¡n ===== */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">

        {/* CÃÂ´ng viÃ¡Â»Âc theo khÃ¡Â»Âi */}
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
            <Building2 size={14} className="text-emerald-600" /> CÃÂ´ng viÃ¡Â»Âc theo khÃ¡Â»Âi
          </h3>
          {perDeptStats.length === 0 ? (
            <div className="text-xs text-slate-400 text-center py-6">ChÃÂ°a cÃÂ³ dÃ¡Â»Â¯ liÃ¡Â»Âu</div>
          ) : (
                ...
                return (
                  <div key={d.id}>
                    ...
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden flex">
                      <div className="bg-emerald-500 h-full" style={{ width: `${pct}%` }} title={`HoÃÂ n thÃÂ nh: ${d.done}`} />
                      <div className="bg-sky-400 h-full" style={{ width: `${d.total > 0 ? d.inProgress/d.total*100 : 0}%` }} title={`ÃÂang lÃÂ m: ${d.inProgress}`} />
                    </div>
                  </div>
                );
              })}
        ...
        {/* TÃ¡ÂºÂ¯c nghÃ¡ÂºÂ½n hiÃ¡Â»Ân tÃ¡ÂºÂ¡i */}
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
            <AlertTriangle size={14} className="text-amber-500" /> TÃ¡ÂºÂ¯c nghÃ¡ÂºÂ½n hiÃ¡Â»Ân tÃ¡ÂºÂ¡i
          </h3>
          {perDeptStats.filter(d => d.overdue > 0).length === 0 ? (
            <div className="text-xs text-emerald-600 text-center py-6 font-medium">Ã¢ÂÂ KhÃÂ´ng cÃÂ³ tÃ¡ÂºÂ¯c nghÃ¡ÂºÂ½n</div>
          ) : (
            <div className="space-y-2">
              {perDeptStats.filter(d => d.overdue > 0).slice(0, 5).map((d, i) => (
                <div key={d.id} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-rose-600 font-semibold tabular-nums">{d.overdue} viÃ¡Â»Âc</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* CÃÂ´ng viÃ¡Â»Âc quÃÂ¡ hÃ¡ÂºÂ¡n */}
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
            <Clock size={14} className="text-rose-500" /> CÃÂ´ng viÃ¡Â»Âc quÃÂ¡ hÃ¡ÂºÂ¡n
          </h3>
          {allTasks.filter(t => t.dueDate && t.dueDate < today && !['done','cancelled','rejected'].includes(t.status)).length === 0 ? (
            <div className="text-xs text-emerald-600 text-center py-6 font-medium">Ã¢ÂÂ KhÃÂ´ng cÃÂ³ viÃ¡Â»Âc quÃÂ¡ hÃ¡ÂºÂ¡n</div>
          ) : (
            <div className="space-y-2">
      ...
      {/* ===== DANH SÃÂCH ÃÂIÃ¡Â»ÂU PHÃ¡Â»ÂI ===== */}
      <section ref={tabSectionRef} className="rounded-xl border border-slate-200 bg-white shadow-sm scroll-mt-20">
        ...
        <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 border-b border-slate-100 bg-slate-50/50">
          ...
          <div className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs text-slate-600 flex-1 min-w-[160px] max-w-xs">
            ...
            <input
              type="text"
              placeholder="TÃÂ¬m kiÃ¡ÂºÂ¿m cÃÂ´ng viÃ¡Â»Âc..."
              value={keyword}
          ...
          {tab !== 'overdue' && (
            <div className="flex items-center gap-1 overflow-x-auto">
              {(['all', 'pending_approval', 'pending', 'in_progress', 'done'] as const).map((s) => (
                ...
                >
                  {s === 'all' ? 'TÃ¡ÂºÂ¥t cÃ¡ÂºÂ£' : STATUS_LABEL[s]}
                </button>
              ))}
            </div>
          )}
          ...
          <div className="ml-auto flex items-center gap-1">
            <button onClick={() => setView('table')} className={`p-1.5 rounded-lg transition ${view === 'table' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-400 hover:bg-slate-100'}`} title="BÃ¡ÂºÂ£ng">
              <ListIcon size={14} />
            </button>
        ...
        <div className="p-4">
          {loading ? (
            <div className="text-center py-12 text-slate-500">
              <Loader2 size={20} className="inline animate-spin mr-2" /> ÃÂang tÃ¡ÂºÂ£iÃ¢ÂÂ¦
            </div>
          ) : error ? (
        ...
        {tasks.length >= 20 && (
          <div className="px-4 py-3 border-t border-slate-100 text-xs text-slate-500 flex items-center justify-between">
            <span>HiÃ¡Â»Ân thÃ¡Â»Â 1Ã¢ÂÂ{tasks.length} trong {tasks.length} cÃÂ´ng viÃ¡Â»Âc</span>
          </div>
        )}
      </section>
      {/* LiÃÂªn khÃ¡Â»Âi section */}
      {tab === 'cross-block' && showLienKhoiTab && (
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            <Users2 size={18} className="text-indigo-600" />
            <h3 className="font-semibold text-slate-800 text-sm">TÃ¡Â»Âng quan liÃÂªn khÃ¡Â»Âi</h3>
            <span className="ml-auto text-xs text-slate-400">Theo dÃÂµi nhiÃ¡Â»Âm vÃ¡Â»Â¥ giao/nhÃ¡ÂºÂ­n giÃ¡Â»Â¯a cÃÂ¡c khÃ¡Â»Âi</span>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            {[
              { label: 'TÃ¡Â»Âng liÃÂªn khÃ¡Â»Âi', value: allTasks.filter(t => t.crossBlock).length, color: 'text-slate-800' },
              { label: 'ÃÂang xÃ¡Â»Â­ lÃÂ½', value: allTasks.filter(t => t.crossBlock && t.status === 'in_progress').length, color: 'text-sky-700' },
              { label: 'ChÃ¡Â»Â phÃ¡ÂºÂ£n hÃ¡Â»Âi', value: allTasks.filter(t => t.crossBlock && t.status === 'pending_approval').length, color: 'text-amber-700' },
              { label: 'QuÃÂ¡ hÃ¡ÂºÂ¡n', value: allTasks.filter(t => t.crossBlock && t.dueDate && t.dueDate < today && !['done','cancelled','rejected'].includes(t.status)).length, color: 'text-rose-700' },
            ].map(c => (
...
// ============================================================================
// TABLE VIEW Ã¢ÂÂ chÃÂ­nh theo mockup
// ============================================================================
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-slate-200">
            {['#', 'CÃÂ´ng viÃ¡Â»Âc', 'LoÃ¡ÂºÂ¡i', 'KhÃ¡Â»Âi chÃ¡Â»Â§ trÃÂ¬', 'PhÃ¡Â»Âi hÃ¡Â»Â£p', 'TrÃ¡ÂºÂ¡ng thÃÂ¡i', 'TiÃ¡ÂºÂ¿n ÃÂÃ¡Â»Â', 'ÃÂang chÃ¡Â»Â', 'Deadline'].map(h => (
        ...
        <tbody className="divide-y divide-slate-100">
          {tasks.map((t, idx) => {
            ...
            // PhÃ¡Â»Âi hÃ¡Â»Â£p: ÃÂ°u tiÃÂªn collaboratorDeptIds/FacilityIds, fallback vÃ¡Â»Â assigneeUserIds
            ...
            const waitingOn =
              t.status === 'pending_approval' ? (t.currentApprover ?? 'NgÃÂ°Ã¡Â»Âi duyÃ¡Â»Ât')
              ...
              >
                ...
                <td className="py-2.5 pr-3 min-w-[200px] max-w-[280px]">
                  <div className="flex items-start gap-2">
                    <span className={`mt-0.5 h-1.5 w-1.5 rounded-full shrink-0 ${PRIORITY_DOT[t.priority] ?? 'bg-slate-300'}`} title={`ÃÂ¯u tiÃÂªn: ${PRIORITY_LABEL[t.priority] ?? t.priority}`} />
                    <div className="min-w-0">
                      <div className="font-semibold text-slate-800 truncate group-hover:text-emerald-700 leading-tight">{t.title}</div>
                      <div className="text-xs text-slate-400 mt-0.5 truncate">#{t.id.slice(-6).toUpperCase()} ÃÂ· {t.createdByName}</div>
                    </div>
                  </div>
                </td>
                <td className="py-2.5 pr-3">
                  <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-semibold ${
                    ...
                    {t.kind === 'proposal' ? 'ÃÂÃ¡Â»Â xuÃ¡ÂºÂ¥t' : 'ÃÂiÃ¡Â»Âu phÃ¡Â»Âi'}
                  </span>
                </td>
                <td className="py-2.5 pr-3">
...
function EmptyState({ tab }: { tab: TabKey }) {
  const msg =
    tab === 'my-tasks' ? 'ChÃÂ°a cÃÂ³ nhiÃ¡Â»Âm vÃ¡Â»Â¥ nÃÂ o ÃÂÃÂ°Ã¡Â»Â£c giao cho bÃ¡ÂºÂ¡n'
    : tab === 'assigned-by-me' ? 'BÃ¡ÂºÂ¡n chÃÂ°a giao viÃ¡Â»Âc nÃÂ o'
    : tab === 'cross-block' ? 'KhÃÂ´ng cÃÂ³ viÃ¡Â»Âc liÃÂªn khÃ¡Â»Âi'
    : tab === 'pending-response' ? 'KhÃÂ´ng cÃÂ³ viÃ¡Â»Âc chÃ¡Â»Â phÃ¡ÂºÂ£n hÃ¡Â»Âi'
    : 'KhÃÂ´ng cÃÂ³ viÃ¡Â»Âc quÃÂ¡ hÃ¡ÂºÂ¡n';
  return (
...
// ============================================================================
// KANBAN VIEW (giÃ¡Â»Â¯ nguyÃÂªn tÃ¡Â»Â« phiÃÂªn bÃ¡ÂºÂ£n cÃÂ©)
// ============================================================================
const KANBAN_COLS: { key: TaskStatus; label: string; bg: string; dot: string }[] = [
  { key: 'pending_approval', label: 'ChÃ¡Â»Â duyÃ¡Â»Ât',   bg: 'bg-amber-50',   dot: 'bg-amber-400' },
  { key: 'pending',          label: 'ChÃ¡Â»Â lÃÂ m',     bg: 'bg-slate-50',   dot: 'bg-slate-400' },
  { key: 'in_progress',      label: 'ÃÂang lÃÂ m',    bg: 'bg-sky-50',     dot: 'bg-sky-500' },
  { key: 'done',             label: 'HoÃÂ n thÃÂ nh',  bg: 'bg-emerald-50', dot: 'bg-emerald-500' },
];
        ...
        return (
          <div key={col.key} className={`flex-shrink-0 w-60 rounded-xl ${col.bg} p-3`}>
            ...
            <div className="space-y-2">
              ...
              {colTasks.length === 0 && (
                <div className="text-xs text-slate-400 text-center py-4">KhÃÂ´ng cÃÂ³</div>
              )}
            </div>
          </div>
        );
