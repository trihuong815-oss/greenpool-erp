...
export async function POST(req: NextRequest) {
  try {
    ...
    // Đảm bảo đủ entries (auto-fill missing với 0)
    ...
    if (old.exists && old.data()?.status === 'approved' && !finalize) {
      return NextResponse.json({ error: 'Batch đã được duyệt — không sửa được. Liên hệ TP_KE.' }, { status: 409 });
    }
    ...
    const data: Record<string, any> = {
      ...
      ...totals,
      note,
      enteredBy: caller.profile.uid,
      enteredByName: caller.actorName,
      enteredAt: old.exists ? (old.data()?.enteredAt ?? now) : now,
      approvedAt: finalize ? now : null,
      updatedAt: now,
    };
    if (isFirstSave) data.createdAt = now;
    ...
    await writeAuditLog({
      ...
      branchId,
      before: old.exists ? { status: old.data()?.status, totalRevenue: old.data()?.totalRevenue } : null,
      after: { status: data.status, totalRevenue: totals.totalRevenue },
    ...
    // Phase 2 sẽ aggregate báo cáo tổng hợp + noti chi tiết hơn.
    if (finalize && !wasApproved) {
      const recipients = await collectRecipientsForFinalize(db, branchId as BranchId);
      if (recipients.length > 0) {
        void sendNotificationEvent({
          type: 'sales_batch_approved', // reuse existing type — Phase 2 sẽ dedicated 'reception_finalized'
          module: 'sales',
          entityId: id,
